...
final class Workspace: Identifiable, ObservableObject {
    ...
    /// Ordered stack of panel IDs whose terminal title should drive the workspace title.
    /// The last element is the active title source. When an agent session ends, its panel
    /// is removed and the previous one resumes. Cleared on app restore.
    var titleSourcePanelIds: [UUID] = []
    ...
    func updatePanelTitle(panelId: UUID, title: String) -> Bool {
        ...
        let shouldSyncToWorkspace = customTitle == nil && (panels.count == 1 || panelId == titleSourcePanelIds.last)
